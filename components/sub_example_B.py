...
class SubInterface_B(silicon.SiOverlay.SubInterface):
...
class Operation_B(silicon.SiLayoutH):
...
class Body_B(silicon.SiTab):
    def __init__(self, parent):
        ...
        self.stack_dir = silicon.SiStack(self.frame)
        self.stack_dir.setTitle('目录设置')

        self.aim = silicon.SiOptionInputBox(self.stack_dir)
        self.aim.setText('备份目标', '需要备份的目录或文件夹',)
        self.aim.setIcon(SiGlobal.icons.get('fi-rr-cloud-upload'))

        self.store = silicon.SiOptionInputBox(self.stack_dir)
        ...
        self.stack_settings = silicon.SiStack(self.frame)
        self.stack_settings.setTitle('备份设置')

        self.frequency = silicon.SiOptionInputBox(self.stack_settings)
        self.frequency.setText('设置备份频率', '每天备份的次数',)
        self.frequency.setIcon(SiGlobal.icons.get('fi-rr-time-fast'))

        self.independency = silicon.SiOptionSwitch(self.stack_settings)
        ...
        self.info_layout_h = silicon.SiLayoutH(self.info_layout_v)
        self.info_layout_h.setCenter(False)

        self.info = silicon.SiInfo(self.info_layout_h)
        self.info.setFixedWidth(320)
        self.info.setType(0)
        self.info.setContent('提示', '· 备份目录将会被以 日期-次数-时间 的格式命名，并分立存放在储存目录下。\n· 当目标名称已存在时，会尝试在上述名称后加编号。')

        self.warning = silicon.SiInfo(self.info_layout_h)
